...
public extension OperationQueue {
    ...
    ///
    ///   **IMPORTANT** – You must ensure that the completion function that is passed into the
    /// `block` is called, or the
...
public extension DispatchQueue {
    ...
    func asyncAfter(
        ...
        let delay = DispatchTime.now() + DispatchTimeInterval.milliseconds(millis.intValue)
...
public extension Anything where Self: AnyObject {
    ...
    ///
    /// - parameter delay: How long of a delay before running the operation. By default, there is no
    /// delay.
