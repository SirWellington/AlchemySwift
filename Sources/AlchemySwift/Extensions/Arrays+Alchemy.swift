...
public extension Array {
    ...
    subscript(safe index: Index) -> Element? {
        return isValidIndex(index) ? self[index] : nil
    }
...
public extension CountableRange where Element == Int {
    var anyElement: Element? {
        let index = Int.randomFrom(minInclusive: lowerBound, maxExclusive: upperBound)
...
public extension Sequence where Element: Hashable {
    ...
    /// of elements in the original sequence. This operation is also more performant, running in
...
public extension Sequence {
    ...
    @discardableResult
    func onEach(_ block: (Element) -> Void) -> [Element] {
        return map {
            block($0)
            return $0
        }
